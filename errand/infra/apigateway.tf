...
resource "aws_apigatewayv2_stage" "default" {
  ...
  default_route_settings {
    # A personal assistant does not receive hundreds of texts a second.
    # This is the cheapest possible brake on somebody hammering the endpoint,
    # and it sits in front of the signature check rather than behind it.
  ...
  access_log_settings {
    ...
    format = jsonencode({
      ...
      # No request body and no From number: the access log is not where
      # message content should end up.
    })
  }
}
